...
class Agent():
    ...
    def make_state(self):
        bod, apple, direct = self.snake.getState()
        obs = self.get_obstacles(bod[0])
        up = self.apple_up(bod[0], apple)
        side = self.apple_side(bod[0], apple)
        state_list = [up] +[side] + obs
        state_tup = tuple(state_list)
        #print(state_tup)
        num = np.ravel_multi_index(state_tup, (3, 3, 2, 2, 2, 2))
        #print(num)
        return num

    def apple_up(self, head, apple):
        if head[1] > apple[1]:
            #print("apple up")
            return 0
        if head[1]== apple[1]:
    ...
    def apple_side(self, head, apple):
        if head[0] > apple[0]:
            #print("apple left")
            return 0
        if head[0]== apple[0]:
    ...
    def check_body(self, head, obs):
        for i in range(1, len(self.snake.body)):
            ...
            if head[1] == snek[1] + CELL_SIZE and head[0] == snek[0]: ##upp
                obs[0] = 1
            if head[0] == snek[0] - CELL_SIZE and head[1] == snek[1]: ##right
                obs[1] = 1
            if head[1] == snek[1] - CELL_SIZE and head[0] == snek[0]: ##down
                obs[2] = 1
            if head[0] == snek[0] + CELL_SIZE and head[1] == snek[1]: ##left 
                obs[3] = 1
        return obs
